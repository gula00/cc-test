...
const modelTestApiMock = vi.hoisted(() => ({
  testProviderPrompt: vi.fn(),
}));
...
vi.mock("@/lib/api/model-test", () => ({
  testProviderPrompt: modelTestApiMock.testProviderPrompt,
}));
...
describe("ClaudeFormFields", () => {
  beforeEach(() => {
    ...
    modelTestApiMock.testProviderPrompt.mockResolvedValue({
  ...
  it("保留 Claude 表单里的测试返回能力", async () => {
    ...
    await waitFor(() => {
      expect(modelTestApiMock.testProviderPrompt).toHaveBeenCalledWith(
        "claude",
        "claude-provider-1",
        "claude-sonnet-4-6",
      );
